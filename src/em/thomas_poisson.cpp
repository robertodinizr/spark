...
void ThomasPoissonSolver1D::efield(const std::vector<double>& phi, std::vector<double>& out) {
    out.resize(m_n);
    efield_extrapolate(phi.data(), out.data(), m_n, m_dx);
}

void ThomasPoissonSolver1D::poisson_thomas(const double* fin,
                                           double* yout,
                                           int n,
                                           double dx,
                                           double ylhs,
...
void ThomasPoissonSolver1D::efield_extrapolate(const double* phi, double* eout, int n, double dx) {
