...
namespace {
    ...
    template <unsigned NX, unsigned NV>
    double max_sigmav_for_cross_section(const CrossSection& cs, const Reactions<NX, NV>& reactions, double projectile_mass, bool slow_projectiles) {
    ...
    template <unsigned NX, unsigned NV>
    double max_sigmav(const Reactions<NX, NV>& reactions, kn::particle::ChargedSpecies<NX, NV>& projectile, bool slow_projectiles) {
        double sigmav = 0.0;
        for(const auto& reaction : reactions)
            sigmav = std::max(sigmav, max_sigmav_for_cross_section(reaction->cross_section, reactions, projectile.m(), slow_projectiles));
        return sigmav;
    }
}
...
MCCReactionSet<NX, NV>::MCCReactionSet(particle::ChargedSpecies<NX, NV>& projectile,
    ...
    m_max_sigmav = max_sigmav(m_config.m_reactions, m_projectile, m_config.m_dyn == RelativeDynamics::SlowProjectile);
};
...
void MCCReactionSet<NX, NV>::react_all() {
    ...
    for(size_t i = 0; i < n_null; i++) {
        ...
        if(m_config.m_dyn == RelativeDynamics::SlowProjectile) {
            ...
            double vth = std::sqrt(kn::constants::kb * m_config.m_target->temperature() / m_projectile.m());
        ...
        for(const auto& reaction : m_config.m_reactions) {
            fr0 = fr1;
            fr1 += collision_frequency(dens_n, interpolate_cross_section(reaction->cross_section, (m_config.m_dyn == RelativeDynamics::SlowProjectile ? 0.5 : 1.0) * kinetic_energy),  kinetic_energy, m_projectile.m()) / nu_prime;
