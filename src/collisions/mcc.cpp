...
template <unsigned NX>
MonteCarloCollisions<NX>::MonteCarloCollisions(DomainConfig config,
                                               std::vector<CollisionReaction>&& cs)
    : m_config(config) {
...
template <unsigned NX>
double MonteCarloCollisions<NX>::frequency_ratio(const CollisionReaction& cs,
                                                 double kinetic_energy) {
    return collision_frequency(m_config.m_n_neutral, interpolate_cross_section(cs, kinetic_energy),
                               kinetic_energy, kn::constants::m_e) /
           m_nu_prime_e;
}

template <unsigned NX>
void MonteCarloCollisions<NX>::isotropic_coll(particle::ChargedSpecies<NX, 3>& species,
                                              size_t idx,
                                              double vmag,
                                              double chi) {
...
template <unsigned NX>
bool MonteCarloCollisions<NX>::electron_excitation_coll(particle::ChargedSpecies<NX, 3>& electrons,
                                                        particle::ChargedSpecies<NX, 3>& ions,
                                                        size_t p_idx,
                                                        double kinetic_energy,
                                                        double threshold) {
    ...
    isotropic_coll(electrons, p_idx, electron_excitation_vmag(kinetic_energy, threshold), chi);
    return true;
}

template <unsigned NX>
bool MonteCarloCollisions<NX>::electron_ionization_coll(particle::ChargedSpecies<NX, 3>& electrons,
                                                        particle::ChargedSpecies<NX, 3>& ions,
                                                        size_t p_idx,
                                                        double kinetic_energy,
                                                        double threshold) {
    ...
    double vmag = electron_ionization_vmag(kinetic_energy, threshold);
    ...
    ions.add(1, [event_pos, ion_mass, neutral_temperature](core::Vec<3>& v, core::Vec<NX>& x) {
        ...
        v = {random::normal(0.0, vtemp), random::normal(0.0, vtemp), random::normal(0.0, vtemp)};
    });
...
template <unsigned NX>
int MonteCarloCollisions<NX>::collide_electrons(particle::ChargedSpecies<NX, 3>& electrons,
                                                particle::ChargedSpecies<NX, 3>& ions) {
            ...
            if (r1 > fr0 && r1 <= fr1) {
                switch (cs.type) {
                    case CollisionType::Ionization: {
                        collided = electron_ionization_coll(electrons, ions, p_idx, kinetic_energy,
                                                            cs.energy_threshold);
                        break;
                    }
                    case CollisionType::Excitation: {
                        collided = electron_excitation_coll(electrons, ions, p_idx, kinetic_energy,
                                                            cs.energy_threshold);
                        break;
                    }
...
bool MonteCarloCollisions<NX>::ions_isotropic_coll(particle::ChargedSpecies<NX, 3>& ions,
    ...
    double vmag =
        std::sqrt(2.0 * kn::constants::e * (kinetic_energy_rel * cos_chi * cos_chi) / ions.m());
...
void MonteCarloCollisions<NX>::collide_ions(particle::ChargedSpecies<NX, 3>& ions) {
    ...
    for (size_t i = 0; i < n_null; i++) {
        ...
        core::Vec<3> v_rand_neutral = {kn::random::normal() * vth, kn::random::normal() * vth,
        ...
        for (const auto& cs : m_ion_cs) {
            fr0 = fr1;
            fr1 += collision_frequency(m_config.m_n_neutral,
                                       interpolate_cross_section(cs, 0.5 * kinetic_energy_rel),
                                       kinetic_energy_rel, ions.m()) /
                   m_nu_prime_i;
